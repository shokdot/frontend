			...
			if (!res.ok) {
				...
				switch (code) {
					case "EMAIL_EXISTS":
						...
						return;
					case "OAUTH_USER":
						setFieldErrors({ email: "This email is registered via GitHub. Try signing in with GitHub instead" });
						return;
					case "USERNAME_EXISTS":
						setFieldErrors({ username: "This username is already taken" });
						return;
					case "WEAK_PASSWORD":
						setFieldErrors({ password: "Password is too weak. Use a mix of letters, numbers, and symbols" });
						return;
					case "VALIDATION_FAILED":
						if (details?.field) {
							const field = details.field as keyof FieldErrors;
							setFieldErrors({ [field]: details.message || "Invalid value" });
						} else {
						...
						<div>
							...
							<div className="group relative">
								...
								<input
									...
									className={`w-full rounded-lg border bg-surface-lighter py-2.5 pl-10 pr-4 text-sm text-white placeholder-zinc-500 outline-none transition-all ${fieldErrors.username ? "border-red-500/50 focus:border-red-500/60 focus:ring-1 focus:ring-red-500/30" : "border-white/10 focus:border-neon-cyan/40 focus:shadow-[0_0_15px_rgba(0,240,255,0.1)] focus:ring-1 focus:ring-neon-cyan/30"}`}
								/>
							</div>
						...
						<div>
							...
							<div className="group relative">
								...
								<input
									...
									className={`w-full rounded-lg border bg-surface-lighter py-2.5 pl-10 pr-4 text-sm text-white placeholder-zinc-500 outline-none transition-all ${fieldErrors.confirmPassword ? "border-red-500/50 focus:border-red-500/60 focus:ring-1 focus:ring-red-500/30" : "border-white/10 focus:border-neon-cyan/40 focus:shadow-[0_0_15px_rgba(0,240,255,0.1)] focus:ring-1 focus:ring-neon-cyan/30"}`}
								/>
							</div>
