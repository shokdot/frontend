...
export default function SettingsPage() {
	...
	// Sync paddle color when profile loads
	useEffect(() => {
		if (profile?.paddleColor) {
			setPaddleColor(profile.paddleColor);
		}
	}, [profile]);
		...
		try {
			switch (activeTab) {
				...
				case "notifications":
					...
					break;
				case "game":
					await updateMyProfile({ paddleColor });
					await loadProfile();
					break;
			}
			setSaved(true);
	...
	const showSaveButton = activeTab === "account" || activeTab === "notifications" || activeTab === "game";
