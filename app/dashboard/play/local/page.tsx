...
const SPEED_CONFIG: Record<GameSpeed, { ballSpeed: number; speedInc: number; maxSpeed: number; paddleSpeed: number; label: string; desc: string }> = {
	easy: { ballSpeed: 4, speedInc: 0.15, maxSpeed: 8, paddleSpeed: 6, label: "Easy", desc: "Slow ball, relaxed pace" },
	medium: { ballSpeed: 5.5, speedInc: 0.3, maxSpeed: 12, paddleSpeed: 7, label: "Medium", desc: "Balanced speed, fair play" },
	hard: { ballSpeed: 7.5, speedInc: 0.5, maxSpeed: 17, paddleSpeed: 9, label: "Hard", desc: "Fast ball, quick reflexes" },
};
							...
							{speeds.map((s) => {
								...
								return (
									<button
										...
										className={`rounded-xl border px-4 py-3 text-left transition-all ${gameSpeed === s
												? sc.active
...
function GameOverScreen({
	...
	const winnerHex = winner === 1 ? p1Color : COLORS.p2;
	const p2Color = COLORS.p2;
		...
		<div className="flex min-h-[calc(100vh-80px)] items-center justify-center p-4">
			<div className="w-full max-w-md text-center">
				<div
					...
					style={{ borderColor: hexToRgba(winnerHex, 0.2), boxShadow: `0 0 40px ${hexToRgba(winnerHex, 0.2)}` }}
				>
					{/* Trophy */}
					<div className="mb-4 flex justify-center">
						<div
							className="flex h-16 w-16 items-center justify-center rounded-full"
							style={{ backgroundColor: hexToRgba(winnerHex, 0.1) }}
						>
							<span style={{ color: winnerHex }}><TrophyIcon className="h-8 w-8" /></span>
						</div>
					</div>

					{/* Winner text */}
					<h1 className="text-3xl font-bold" style={{ color: winnerHex }}>
						Player {winner} Wins!
					</h1>
					...
					<div className="mt-6 flex items-center justify-center gap-6">
						...
						<div className="text-center">
							...
							<p className="mt-1 text-4xl font-bold" style={{ color: p2Color }}>{score2}</p>
						</div>
					</div>
		...
		<div className="flex min-h-[calc(100vh-80px)] flex-col items-center justify-center gap-4 p-4">
			...
			<div className="flex w-full max-w-[800px] items-center justify-between px-2">
				<div className="flex items-center gap-3">
					...
					<span className="text-sm font-medium" style={{ color: p1Color }}>Player 1</span>
					...
					<div className="absolute inset-0 flex flex-col items-center justify-center gap-4 rounded-xl bg-black/60 backdrop-blur-sm">
						...
						<div className="mt-2 flex gap-3">
							<button
								...
								className="flex items-center gap-2 rounded-xl bg-accent px-5 py-2.5 text-sm font-semibold text-white shadow-[0_0_20px_rgba(139,92,246,0.4)] transition-all hover:shadow-[0_0_35px_rgba(139,92,246,0.6)]"
							>
								<PlayIcon className="h-4 w-4" />
								Resume
							</button>
