...
const BALL_SIZE = 10;   // server hitbox size (top-left coords)
const BALL_R = 8;       // visual radius (drawn as circle)
...
function drawGame(
	...
	// The player is always rendered on the left for consistency
	...
	// Ball center — server sends top-left coords; convert to center
	...
	// Mirror for player 2 so they see themselves on the left
	if (playerNumber === 2) {
		ballCX = w - ballCX;
		ballVX = -ballVX;
	}
	...
	// My paddle (left) — with offset from wall
	...
	// Opponent paddle (right) — with offset from wall
			...
			<div className="w-full max-w-md text-center">
				...
				>
					...
					<div className="mt-8">
						<Link
							href="/dashboard/play"
							className="inline-flex items-center justify-center gap-2 rounded-xl bg-neon-cyan px-8 py-3 text-sm font-semibold text-surface shadow-[0_0_20px_rgba(0,240,255,0.4)] transition-all hover:shadow-[0_0_35px_rgba(0,240,255,0.6)]"
						>
							Go to Play
						</Link>
					</div>
...
function OnlineGameScreen({
	...
	// Rendering loop - renders the latest server state.
	// `phase` is in the dep array so the loop restarts when the canvas
	// first appears (phase goes from "connecting" → "waiting").
	useEffect(() => {
		...
		function loop() {
			...
			if (state && ctx) {
				try {
					...
					// Prevent a rendering error from killing the loop
				}
			}
	...
	// Keyboard input — the most recently pressed direction key wins so that
	// switching from up→down (or vice-versa) feels instant without needing
	// to release the previous key first.
	useEffect(() => {
		...
		function handleKeyDown(e: KeyboardEvent) {
			...
			// Immediately apply the direction of the key that was just pressed
		...
		function handleKeyUp(e: KeyboardEvent) {
			...
			// On release, check what's still held and apply that direction
	...
	// Fetch opponent profile from the players array sent by the server
	...
	useEffect(() => {
		...
		// Fetch own userId so we can identify the opponent
		getMyProfile()
			.then((res) => {
			...
			try {
				...
				switch (data.type) {
					...
					case "reconnected":
						...
						if (data.players) fetchOpponentName(data.players);
						stateRef.current = data.state;
						// Don't jump to playing – wait for countdown
						if (disconnectTimerRef.current) {
							clearInterval(disconnectTimerRef.current);
							disconnectTimerRef.current = null;
						}
					...
					case "game_resumed":
						// Both reconnected – countdown will follow
						if (disconnectTimerRef.current) {
							clearInterval(disconnectTimerRef.current);
							disconnectTimerRef.current = null;
						}
		...
		ws.onclose = (event) => {
			// Normal closure (1000) after game_end is fine
			if (phaseRef.current === "game_over") return;
			if (event.code === 1000) return;

			// Handle specific server close codes with appropriate messages
			if (event.code === 4004) {
				// Game not found — it has already ended or never existed
			...
			// During connecting/waiting the game-service may not be ready yet.
			// Avoid flashing an error screen by showing the connecting state
			// and only transitioning to error after a short grace period.
		...
		ws.onerror = () => {
			// onerror is always followed by onclose – let onclose decide the phase.
		};
		...
		<div className="flex min-h-[calc(100vh-80px)] flex-col items-center justify-center gap-4 p-4">
			...
			<div className="flex w-full max-w-[800px] items-center justify-between px-2">
				...
				<div className="flex items-center gap-2 text-xs text-zinc-500">
					...
					{phase === "waiting" && (
						<span className="flex items-center gap-1.5 rounded-full border border-neon-pink/20 bg-neon-pink/10 px-3 py-1 text-neon-pink">
			...
			<div className="relative w-full max-w-[800px]">
				<canvas
					...
					width={CANVAS_W}
					height={CANVAS_H}
					className="w-full rounded-xl border border-white/10 shadow-[0_0_40px_rgba(224,64,251,0.06)]"
				...
				{phase === "connecting" && (
					<div className="absolute inset-0 flex flex-col items-center justify-center gap-4 rounded-xl bg-black/70 backdrop-blur-sm">
						<div className="relative">
							...
							<GlobeIcon className="absolute top-1/2 left-1/2 h-6 w-6 -translate-x-1/2 -translate-y-1/2 text-zinc-400" />
						</div>
...
function OnlineGamePageInner() {
	...
	return (
		<div className="relative">
			<div className="pointer-events-none absolute top-0 right-0 h-[400px] w-[400px] rounded-full bg-neon-pink/5 blur-[150px]" />
